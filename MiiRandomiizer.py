...
def button2():
    ...
    sg.Popup('Mii SwordFighter', random.choice(SNeutralSpecial), random.choice(SSideSpecial), random.choice(SUpSpecial),
...
# Button Layout
layout = [
              [sg.Button('Mii Brawler'), sg.Button('Mii Swordfighter'), sg.Button('Mii Gunner'), sg.Button('Random')]
             ]
# Shows Window
window = sg.Window('Mii Fighter Randomiizer', layout)

# Event loop
while True:
    event, value = window.Read()
    # Takes appropriate action based on button
    if event == 'Mii Brawler':
        button1()
    elif event == 'Mii Swordfighter':
        button2()
